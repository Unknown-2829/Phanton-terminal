...
NEON_GREEN="${ESC}[38;5;118m"   NEON_PURPLE="${ESC}[38;5;129m"
NEON_CYAN="${ESC}[38;5;87m"     ELECTRIC_BLUE="${ESC}[38;5;39m"
HOT_PINK="${ESC}[38;5;205m"     GOLD="${ESC}[38;5;220m"
BLOOD_RED="${ESC}[38;5;196m"    BRIGHT_RED="${ESC}[1;91m"
WHITE="${ESC}[1;37m"            GRAY="${ESC}[38;5;244m"
DARK_GRAY="${ESC}[38;5;240m"    ORANGE="${ESC}[38;5;208m"

# ═══════════════════════════════════════════════════
# UNICODE DETECTION
# Works on all Linux terminals + Termux + macOS
# ═══════════════════════════════════════════════════
_supports_unicode() {
    case "${TERM:-}" in
        xterm*|rxvt*|screen*|tmux*|vte*|alacritty*|foot*|kitty*) return 0 ;;
    esac
...
if _supports_unicode; then
    SKULL="☠";  SHIELD="░";  LOCK="■"
    HIGH_VOLTAGE="⚡"; SUCCESS="✔"; FAILURE="✘"
    PROMPT="»"; CPU="⚙"; RAM="☰"
    HLINE="═"; VLINE="║"; TOP_LEFT="╔"; TOP_RIGHT="╗"
    BOTTOM_LEFT="╚"; BOTTOM_RIGHT="╝"; T_LEFT="╠"; T_RIGHT="╣"
    BLOCK="█"; BLOCK_EMPTY="░"
    # FIX: visual width of each box char = 1, but bash ${#} counts 3 bytes
    # So we track BOX_CHAR_W=1 and use _vlen() for accurate padding
    BOX_CHAR_BYTES=3
else
    SKULL="[X]"; SHIELD="[#]"; LOCK="[=]"
    HIGH_VOLTAGE="[!]"; SUCCESS="[+]"; FAILURE="[x]"
    PROMPT=">"; CPU="[C]"; RAM="[R]"
    HLINE="="; VLINE="|"; TOP_LEFT="+"; TOP_RIGHT="+"
    BOTTOM_LEFT="+"; BOTTOM_RIGHT="+"; T_LEFT="+"; T_RIGHT="+"
    BLOCK="#"; BLOCK_EMPTY="-"
...
# ───────────────────────────────────────────────────
# FIX: Visual string length (strips ANSI + counts
#      Unicode chars correctly via wc -m)
# This is the core fix for Termux box alignment bug.
# bash ${#var} returns BYTE count for UTF-8 strings,
# causing right-border to appear off-screen.
# ───────────────────────────────────────────────────
_vlen() {
    # Strip ANSI escape codes, then count characters (not bytes)
    printf '%s' "$1" | sed 's/\x1b\[[0-9;]*[a-zA-Z]//g' | wc -m | tr -d ' '
}
...
load_config() {
    ...
    THEME="Phantom";        AUTO_CHECK_UPDATES=true
    SILENT_UPDATE=true;     UPDATE_CHECK_DAYS=1
...
get_theme_colors() {
    case "$THEME" in
        "Unknown")
            ...
            QUOTES=(
                'Hidden in plain sight...'   'Anonymous by design.'
                'The unknown cannot be traced.' 'Identity: NULL'
                'No name. No trace. No limits.' 'In anonymity, we trust.'
        ...
        *)
            ...
            QUOTES=(
                'In the shadows, we code...'  'Access denied. Until now.'
...
write_centered() {
    ...
    get_terminal_size
    local vl; vl=$(_vlen "$text")
    local pad=$(( (TERM_WIDTH - vl) / 2 ))
    [[ $pad -lt 0 ]] && pad=0
    printf "%${pad}s%b%s%b\n" "" "$color" "$text" "$RESET"
}
...
# ═══════════════════════════════════════════════════
# ANIMATION: SECURITY BARS
# ═══════════════════════════════════════════════════
show_security_loading_bar() {
    local desc="$1" steps="${2:-8}" color="${3:-$PRIMARY}"
    local bar_w=$steps
    [[ "$IS_SMALL_SCREEN" == "true" ]] && bar_w=$(( steps > 4 ? steps / 2 : steps ))
...
# Sparse columns + trail fade = Windows-parity look
# ═══════════════════════════════════════════════════
show_multicolor_matrix() {
    local duration="${1:-2}"
    clear_screen; get_terminal_size

    local col_step=2 sleep_time=0.030
    case "$PLATFORM" in
        termux) col_step=3; sleep_time=0.055; duration=$(( duration > 2 ? 2 : duration )) ;;
        macos)  col_step=2; sleep_time=0.030 ;;
    esac
    ...
    while [[ $SECONDS -lt $end_time ]]; do
        for ((col=0; col<TERM_WIDTH; col+=col_step)); do
            ...
            local color="${MATRIX_COLORS[$(( col % ${#MATRIX_COLORS[@]} ))]}"
            local row=${drops[$col]:-1}

            if [[ $row -ge 1 && $row -lt $TERM_HEIGHT ]]; then
                move_cursor "$row" "$col";           printf "%b%s%b" "$color" "$char" "$RESET"
                local lead=$(( row+1 ))
                [[ $lead -lt $TERM_HEIGHT ]] && { move_cursor "$lead" "$col"; printf "%b%s%b" "$WHITE" "$char" "$RESET"; }
                local trail=$(( row-2 ))
                [[ $trail -ge 1 ]]           && { move_cursor "$trail" "$col"; printf "%b%s%b" "$DARK_GRAY" "$char" "$RESET"; }
            fi
...
# ═══════════════════════════════════════════════════
show_glitch_reveal() {
    local art="$1"; clear_screen; get_terminal_size
    mapfile -t lines <<< "$art"

    local max_w=0
    for line in "${lines[@]}"; do [[ ${#line} -gt $max_w ]] && max_w=${#line}; done
    local sc=$(( (TERM_WIDTH - max_w) / 2 )); [[ $sc -lt 1 ]] && sc=1
    local sr=$(( (TERM_HEIGHT - ${#lines[@]}) / 2 - 1 )); [[ $sr -lt 2 ]] && sr=2
    local glitch='!@#$%_+-=:;,.?/\\|~^'
...
write_gradient_logo() {
    local art="$1"; mapfile -t lines <<< "$art"
    get_terminal_size
    local max_w=0
    for line in "${lines[@]}"; do [[ ${#line} -gt $max_w ]] && max_w=${#line}; done
    local sc=$(( (TERM_WIDTH - max_w) / 2 )); [[ $sc -lt 0 ]] && sc=0
    local n=0
    for line in "${lines[@]}"; do
        local c
        [[ "$GRADIENT_TEXT" == "true" ]] \
...
_cpu_linux() {
    ...
    local l1 l2; read -r l1 < /proc/stat; sleep 0.1; read -r l2 < /proc/stat
    local -a a=($l1) b=($l2)
    local t1=0 t2=0
...
write_usage_bar() {
    local indent="$1" label="$2" usage="$3" bar_w="${4:-28}"
    [[ "$usage" == "N/A" ]] && return
    local bc="$NEON_GREEN"
    [[ $usage -gt 80 ]] && bc="$BLOOD_RED"
    [[ $usage -gt 60 && $usage -le 80 ]] && bc="$ORANGE"
    local filled=$(( usage * bar_w / 100 ))
    [[ $filled -gt $bar_w ]] && filled=$bar_w
    local bar=""
    for ((i=0; i<filled;        i++)); do bar+="$BLOCK";       done
    for ((i=filled; i<bar_w;    i++)); do bar+="$BLOCK_EMPTY"; done
...
# DASHBOARD
# ───────────────────────────────────────────────────
# KEY FIX (Termux box alignment):
#   All padding calculations now use _vlen() which
#   counts visible characters (via wc -m after stripping
#   ANSI codes), NOT bash ${#var} which returns byte-count
#   for UTF-8 strings — causing right-borders to appear
#   far off-screen on Termux / narrow terminals.
# ═══════════════════════════════════════════════════
show_dashboard() {
    clear_screen; get_terminal_size

    local user="${USER:-$(whoami 2>/dev/null || echo 'user')}"
    local host="${HOSTNAME:-$(hostname 2>/dev/null || echo 'localhost')}"; host="${host%%.*}"
    ...
    # Uptime — works on Linux, macOS, Termux
    ...
    # ── Box sizing ──────────────────────────────────
    # box_width = number of VISIBLE characters wide
    local box_width=67
    local min_w=$(( $(_vlen "$TITLE") + 14 ))
    [[ $box_width -lt $min_w ]] && box_width=$min_w
    [[ $TERM_WIDTH -lt $(( box_width + 4 )) ]] && box_width=$(( TERM_WIDTH - 4 ))
    [[ $box_width -lt 30 ]] && box_width=30

    local inner=$(( box_width - 2 ))   # chars between left and right borders

    local padding=$(( (TERM_WIDTH - box_width) / 2 ))
    [[ $padding -lt 0 ]] && padding=0
    local ind; ind=$(printf "%${padding}s" "")

    # Build horizontal line: exactly (inner) copies of HLINE
    local hln; hln=$(printf "%${inner}s" "" | tr ' ' "$HLINE")

    # ── Box helpers ─────────────────────────────────

    # Print a full-width border line
    _border() { printf "%s%b%s%s%s%b\n" "$ind" "$PRIMARY" "$1" "$hln" "$2" "$RESET"; }

    # Print a content row:
    #   _row "Label: " "value" COLOR
    # Padding is calculated using _vlen so Unicode box chars
    # do NOT throw off the right-border position on Termux.
    _row() {
        local lbl="$1" val="$2" vc="$3"
        # Truncate value if it would overflow the box
        local content="  ${lbl}${val}"
        local cvl; cvl=$(_vlen "$content")
        while [[ $cvl -gt $(( inner - 2 )) && ${#val} -gt 1 ]]; do
            val="${val:0:$(( ${#val}-1 ))}"
            content="  ${lbl}${val}"
            cvl=$(_vlen "$content")
        done
        # Right-pad with spaces so right border lines up
        local rpad=$(( inner - cvl - 1 ))
        [[ $rpad -lt 0 ]] && rpad=0
    ...
    # ── Draw box ────────────────────────────────────
    _border "$TOP_LEFT" "$TOP_RIGHT"

    # Title row — centered inside box
    local title_str="$SKULL $TITLE v$SCRIPT_VERSION $SKULL"
    local tvl; tvl=$(_vlen "$title_str")
    local tpad=$(( inner - tvl - 1 )); [[ $tpad -lt 0 ]] && tpad=0
    ...
    if [[ "$SHOW_SYSTEM_INFO" == "true" ]]; then
        ...
        if [[ "$cpu_u" != "N/A" || "$ram_u" != "N/A" ]]; then
            local bar_w=$(( inner - 22 )); [[ $bar_w -lt 8 ]] && bar_w=8
    ...
    # Quote row — also uses _vlen for correct padding
    local qi=$(( RANDOM % ${#QUOTES[@]} ))
    local quote="${QUOTES[$qi]}"
    local qvl; qvl=$(_vlen "$quote")
    local qpad=$(( inner - qvl - 2 )); [[ $qpad -lt 0 ]] && qpad=0
    printf "%s%b%s %b%s%*s%b%s%b\n" \
        "$ind" "$PRIMARY" "$VLINE" \
        "$GRAY" "$quote" \
        "$qpad" "" \
    ...
    _border "$BOTTOM_LEFT" "$BOTTOM_RIGHT"
    echo ""

    local hcmd="phantom-help"
    [[ "$THEME" == "Unknown" ]] && hcmd="unknown-help"
    printf "%s%b%s%b\n" "$ind" "$DARK_GRAY" \
        "Type '${GOLD}${hcmd}${DARK_GRAY}' for commands." "$RESET"
    echo ""
}
...
phantom-help() {
    ...
    echo "  ${GOLD}${p}-config --edit${WHITE} - Edit config in \$EDITOR${RESET}"
    echo "  ${GOLD}${p}-matrix${WHITE}        - Matrix rain only${RESET}"
    echo "  ${GOLD}${p}-dash${WHITE}          - Show dashboard${RESET}"
    echo "  ${GOLD}${p}-update${WHITE}        - Check for updates${RESET}"
    echo ""
    echo "  ${GRAY}Hidden: ${p}-chosen  ${p}-2829  ${p}-secrets${RESET}"
    echo ""
}
...
phantom-config() {
    ...
    if [[ "$1" == "-edit" || "$1" == "--edit" ]]; then
        ${EDITOR:-nano} "$CONFIG_FILE"; return
    fi
    echo ""
    echo "${NEON_CYAN}Config: $CONFIG_FILE${RESET}"; echo ""
    for pair in \
        "Theme:$THEME" "AnimationEnabled:$ANIMATION_ENABLED" \
        "MatrixMode:$MATRIX_MODE" "MatrixDuration:$MATRIX_DURATION" \
        "SecurityLoadSteps:$SECURITY_LOAD_STEPS" "GlitchIntensity:$GLITCH_INTENSITY" \
        "ShowSystemInfo:$SHOW_SYSTEM_INFO" "GradientText:$GRADIENT_TEXT" \
        "AutoCheckUpdates:$AUTO_CHECK_UPDATES"
    do
        local k="${pair%%:*}" v="${pair#*:}"
        echo "  ${GOLD}${k}${WHITE}: ${v}${RESET}"
    done
    echo ""; echo "${DARK_GRAY}phantom-config --edit  to change settings${RESET}"; echo ""
}
...
phantom-theme() {
    ...
    save_config
    echo "${NEON_GREEN}Theme set to: $THEME — run 'phantom-reload' to apply.${RESET}"
}
...
phantom-update() {
    ...
    if [[ "$latest" > "$SCRIPT_VERSION" ]]; then
        ...
        echo "${NEON_GREEN}Already on latest (v$SCRIPT_VERSION)${RESET}"
    fi
}
...
phantom-secrets() {
    ...
    echo ""
    [[ $found -eq $total ]] && \
        write_centered "${GOLD}⚡ ACHIEVEMENT UNLOCKED — Master Secret Hunter ⚡${RESET}" "$GOLD" || \
        write_centered "${DARK_GRAY}Hint: try phantom-chosen, phantom-2829${RESET}" "$DARK_GRAY"
    echo ""; _save_secret "secrets"
}
...
set_phantom_prompt() {
    ...
    _phantom_ps1() {
        ...
        PS1="${PRIMARY}${USER:-user}${DARK_GRAY}@${NEON_CYAN}${path_d}${git_b}${RESET}\n${sym} ${PRIMARY}${PROMPT}${RESET} "
    }
